...
- The planimetric denominator is the landform's whole area, but the procedure asks for its "total green (forested) portion". It reads low on landforms carrying non-forested ground, and fixing it needs a VRI rank-1 mask through bcdatamapper. Say so rather than quietly narrowing the denominator with partial data.
