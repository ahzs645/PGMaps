...
function MobileCircleCanvas({
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      drawHandle(context, centerScreen.x, centerScreen.y, '#ffffff', 4)
  ...
  const pointFromEvent = (event: ReactPointerEvent<HTMLCanvasElement>): [number, number] | null => {
  ...
  const handlePointerDown = (event: ReactPointerEvent<HTMLCanvasElement>) => {
    if (!canAdjust || !map || !edgePoint || !canvasRef.current) return
    const rect = canvasRef.current.getBoundingClientRect()
    const edgeScreen = map.project(edgePoint)
    const distance = Math.hypot(edgeScreen.x - (event.clientX - rect.left), edgeScreen.y - (event.clientY - rect.top))
  ...
  return (
    <canvas
      ...
      aria-hidden="true"
      className={cn(
        'absolute inset-0 z-[300] h-full w-full md:hidden',
        canAdjust ? 'pointer-events-auto touch-none cursor-grab active:cursor-grabbing' : 'pointer-events-none',
      )}
...
function drawHandle(context: CanvasRenderingContext2D, x: number, y: number, color: string, radius: number) {
  context.setLineDash([])
