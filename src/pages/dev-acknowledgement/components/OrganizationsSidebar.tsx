...
export function OrganizationsSidebar({ selectedId, onSelect }: OrganizationsSidebarProps) {
  ...
  return (
    <aside className="flex flex-col overflow-hidden rounded-lg border bg-white shadow-sm lg:sticky lg:top-4 lg:max-h-[calc(100vh-2rem)] lg:self-start">
      <div className="border-b p-3">
        <h2 className="text-sm font-semibold">Organizations</h2>
        <p className="mt-0.5 text-[11px] leading-4 text-slate-500">{organizations.length} presets · click one to preview on the map</p>
        ...
        {filtered.map((org) => (
          ...
          >
            ...
            <div className="truncate text-[10px] text-slate-500">
              {humanize(org.sector)} · {org.acknowledges.length ? `${org.acknowledges.length} Nation${org.acknowledges.length === 1 ? '' : 's'}` : 'region-wide'}
            </div>
          </button>
        ))}
