    ...
    .map((restaurant) => {
      ...
      return {
        ...
        details: [
          ...
          { label: 'Inspections', value: inspectionCount.toLocaleString(DEFAULT_LOCALE) },
          { label: 'Violations', value: violationCount.toLocaleString(DEFAULT_LOCALE) },
        ],
...
export function buildCensusAreaItems(units: CensusUnit[], level: CensusAreaLevel): ExplorerItem[] {
  ...
  units.forEach((unit) => {
    ...
    items.push({
      ...
      summary: `Population ${population.toLocaleString(DEFAULT_LOCALE)} with density ${density.toLocaleString(DEFAULT_LOCALE, { maximumFractionDigits: 1 })} /km².`,
      bounds,
      geometry: unit.geometry,
      details: [
        { label: 'Population', value: population.toLocaleString(DEFAULT_LOCALE) },
        { label: 'Density', value: density.toLocaleString(DEFAULT_LOCALE, { maximumFractionDigits: 1 }) },
        { label: 'Households', value: (unit.households || 0).toLocaleString(DEFAULT_LOCALE) },
        { label: 'Dwellings', value: (unit.dwellings || 0).toLocaleString(DEFAULT_LOCALE) },
...
export function buildCensusBlockItems(units: CensusUnit[]): ExplorerItem[] {
  ...
  units.forEach((unit) => {
    ...
    items.push({
      ...
      summary: `Census block with population ${population.toLocaleString(DEFAULT_LOCALE)}.`,
      bounds,
      geometry: unit.geometry,
      details: [
        { label: 'Population', value: population.toLocaleString(DEFAULT_LOCALE) },
    ...
    .map((stop) => {
      ...
      return {
        ...
        details: [
          ...
          { label: 'Weekday trips', value: stop.weekdayTrips.toLocaleString(DEFAULT_LOCALE) },
  ...
  return features
    ...
    .map((feature, index) => {
      ...
      return {
        ...
        subtitle: `${crashCount.toLocaleString(DEFAULT_LOCALE)} crashes | ${props.datasetTitle}`,
        relevance,
        relevanceBreakdown: [
          { label: 'Base', points: 20 },
        ...
        details: [
          ...
          { label: 'Crashes', value: crashCount.toLocaleString(DEFAULT_LOCALE) },
    ...
    .map((feature) => {
      ...
      return {
        ...
        details: [
          ...
          { label: 'Quantity', value: quantity.toLocaleString(DEFAULT_LOCALE) },
...
export function buildBcAssessmentItems(properties: Property[]): ExplorerItem[] {
  ...
  properties.forEach((property) => {
    ...
    items.push({
      ...
      subtitle: `${property.category} | $${(property.totalAssessed || 0).toLocaleString(DEFAULT_LOCALE)}`,
      relevance,
      relevanceBreakdown: [
        { label: 'Base', points: 20 },
      ...
      summary: `${property.category} parcel assessed at $${(property.totalAssessed || 0).toLocaleString(DEFAULT_LOCALE)}.`,
      bounds,
      geometry: property.geometry,
      details: [
        { label: 'Address', value: formatNullableText(property.address, 'Unknown') },
        { label: 'Category', value: property.category },
        { label: 'Total assessed', value: `$${(property.totalAssessed || 0).toLocaleString(DEFAULT_LOCALE)}` },
        { label: 'Land', value: `$${(property.totalLand || 0).toLocaleString(DEFAULT_LOCALE)}` },
        { label: 'Building', value: `$${(property.totalBuilding || 0).toLocaleString(DEFAULT_LOCALE)}` },
