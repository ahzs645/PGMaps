...
function geometryBounds(geometry: GeoJSON.Geometry): GeometryBounds | null {
  ...
  if (geometry.type === 'LineString') {
    geometry.coordinates.forEach(([lng, lat]) => expandBounds(bounds, lng, lat))
  } else if (geometry.type === 'MultiLineString') {
    geometry.coordinates.forEach((line) => line.forEach(([lng, lat]) => expandBounds(bounds, lng, lat)))
  } else if (geometry.type === 'Polygon') {
    geometry.coordinates.forEach((ring) => scanRing(ring))
...
export default function ExplorerSection() {
  ...
  const restaurantItems = useMemo<ExplorerItem[]>(() => {
    return restaurants
      ...
      .map((restaurant) => {
        ...
        const inspectionCount = restaurant.inspections?.length || 0
        const violationCount = countInspectionViolations(restaurant.inspections)
        const rating = (restaurant.current_hazard_rating || restaurant.hazard_rating || 'Unknown') as HazardRating
        const relevance = clampScore(
          20
          + hazardWeight(rating)
          + Math.min(violationCount * 2.5, 40)
          + Math.min(inspectionCount * 1.6, 20)
        )
  ...
  const amenityItems = useMemo<ExplorerItem[]>(() => {
    return amenities
      ...
      .map((amenity) => {
        const relevance = clampScore(34 + (amenity.parkName ? 10 : 0) + (amenity.type ? 10 : 0))
        const geometry: GeoJSON.Point = {
          type: 'Point',
    ...
    trails
      ...
      .forEach((trail) => {
        ...
        const relevance = clampScore(
          24
          + (normalizedLength * 50)
          + (trail.winterMaintenance ? 12 : 4)
          + trailClassWeight(trail.userClass)
        )
  ...
  const parkItems = useMemo<ExplorerItem[]>(() => {
    ...
    parks.forEach((park) => {
      ...
      const relevance = clampScore(
        ...
        + (normalizedArea * 48)
        + (park.developed ? 14 : 6)
        + classificationWeight(park.classification)
      )
  ...
  const censusDaItems = useMemo<ExplorerItem[]>(() => {
    ...
    unitsByLevel.da.forEach((unit) => {
      ...
      const relevance = clampScore(22 + (densityNorm * 44) + (popNorm * 32))
    ...
    return items
  }, [censusDaRange.densityMax, censusDaRange.densityMin, censusDaRange.populationMax, censusDaRange.populationMin, unitsByLevel.da])
  ...
  }, [amenityItems, censusDaItems, monitorItems, parkItems, restaurantItems, trailItems])
  ...
  const filteredItems = useMemo(() => {
    ...
    const filtered = allItems.filter((item) => {
      ...
      return [item.name, item.subtitle, item.summary]
        .join(' ')
        .toLowerCase()
        .includes(query)
    })
    ...
    filtered.sort((a, b) => {
      if (sortMode === 'name') {
        return a.name.localeCompare(b.name) || b.relevance - a.relevance
      }
      return b.relevance - a.relevance || a.name.localeCompare(b.name)
    })

    return filtered
  }, [allItems, datasetSet, geometrySet, searchQuery, sortMode])
  ...
  const toggleDataset = useCallback((datasetId: ExplorerDatasetId) => {
    setActiveDatasetIds((current) => {
      if (current.includes(datasetId)) {
        const next = current.filter((entry) => entry !== datasetId)
        return next
      }
