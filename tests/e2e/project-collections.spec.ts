...
for (const width of [320, 390, 1024, 1440]) {
  test(`climate folder supports browsing, search, links, and return at ${width}px`, async ({ page }, testInfo) => {
    ...
    // Folders are ordinary list entries, pinned ahead of the projects.
    const folderEntry = page.locator('[data-entry="folder"]')
    const folder = folderEntry.getByRole('link').first()
    await expect(folderEntry).toContainText('B.C. Climate & Health')
    await expect(folderEntry).toContainText('20 projects')
    await expect(folder).toHaveAttribute('href', folderUrl)
    await expect(page.getByRole('status')).toContainText('1–12 of 19 · 1 folder · 18 projects')
    await folder.click()
    ...
    const rows = page.locator('[data-entry="project"]')
    ...
    await expect(folderEntry).toContainText('1 matching')
    await expect(folderEntry).toContainText('20')
    await folder.click()
    await expect(rows).toHaveCount(1)
    await rows
      .first()
...
test('unknown folder offers a way back instead of showing unrelated projects', async ({ page }) => {
  ...
  await page
    ...
    .click()
  await expect(page.locator('[data-entry="folder"]')).toBeVisible()
})
